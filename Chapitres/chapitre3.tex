% !TEX root = ../main.tex
% ══════════════════════════════════════════════════════════════
\chapter{Besoins du système et choix technologiques}
% ══════════════════════════════════════════════════════════════
\begingroup
\let\clearpage\relax
...
Ce chapitre détaille les besoins fonctionnels et non fonctionnels du système, présente les profils utilisateurs et le périmètre de réalisation, puis justifie les choix technologiques retenus pour chaque composant du stack.
...
Les fonctionnalités intelligentes sont identifiées comme des extensions V2. Elles sont conservées dans l'analyse comme pistes d'évolution, mais ne font pas partie du périmètre MVP engagé.

% Fonctionnalités V2 prévues ultérieurement :
% \begin{itemize}
...
\textbf{L'étudiant} consulte les destinations disponibles, suit l'état de sa candidature, déclare ses expériences complémentaires et vérifie à tout moment sa conformité CTI. Les recommandations personnalisées et l'assistant virtuel restent des extensions V2 conditionnelles.
...
Ces fonctionnalités V2 sont des \textit{Could Have}. Elles ne seront intégrées à un sprint que si le MVP est réalisé et validé dans les sprints prioritaires, avec une capacité restante suffisante.
...
\hline
Adapté à ce projet & \xmark & Partiel & \cmark \\
\hline
\end{tabular}
...
\subsubsection{Langage ubiquitaire} 
...
Le tableau suivant récapitule les critères évalués pour chacune des trois solutions, afin de faciliter la comparaison et de justifier le choix retenu.
...
\hline
Rendu côté serveur (SSR) & \xmark & Partiel & \cmark \\
\hline
...
\hline
Intégration SSO CAS/OIDC & \cmark & Limitée & \cmark \\
\hline
...
Le tableau suivant récapitule les critères évalués pour chacune des deux
...
\hline
Support async natif & Partiel & \cmark \\
\hline
...
\hline
Maturité et communauté & \cmark & Partiel \\
\hline
...
Le tableau suivant récapitule les critères évalués pour chacune des trois 
solutions, afin de faciliter la comparaison et de justifier le choix retenu.
...
\hline
Transactions ACID multi-tables & \cmark & Partiel & \cmark \\
\hline
Support JSONB natif + indexation & Partiel & \cmark & \cmark \\
\hline
File d'attente de tâches & \xmark & \xmark & \cmark \\
\hline
Intégration ORM Django & \cmark & Partiel & \cmark \\
\hline
...
\hline
Garanties ACID & \xmark & Partiel & \cmark \\
\hline
...
\hline
Planificateur intégré & Partiel & \cmark \\
\hline
Supervision via admin Django & \xmark & \cmark \\
\hline
Adapté au volume du projet & Partiel & \cmark \\
\hline
Maturité et communauté & \cmark & Partiel \\
\hline
Intégration native avec Django & Partiel & \cmark \\
\hline
\end{tabular}
...
\hline
Simplicité de configuration & \cmark & Partiel \\
\hline
\end{tabular}
...
Le tableau suivant récapitule les critères évalués pour chacune des
...
Le tableau suivant récapitule les critères évalués pour chacun des
...
\hline
Configuration lisible (fichiers texte) & \cmark & Partiel \\
\hline
Gestion SSL native & Partiel & \cmark \\
\hline
Automatisation Docker & Partiel & \cmark \\
\hline
Prévisibilité en environnement institutionnel & \cmark & Partiel \\
\hline
...
\hline
Compatible avec les migrations Django & Partiel & \cmark \\
\hline
Déclaration dans les modèles & \xmark & \cmark \\
\hline
Aucune configuration PostgreSQL séparée & \xmark & \cmark \\
\hline
Efficacité à très grande échelle & \cmark & Partiel \\
\hline
\end{tabular}
...
\hline
Optimalité pour les étudiants & Partiel & \cmark \\
\hline
Explicabilité des résultats & Partiel & \cmark \\
\hline
Simplicité d’implémentation & \cmark & Partiel \\
\hline
\end{tabular}
...
\hline
Volume de données requis & \cmark & \xmark & Partiel & Partiel \\
\hline
Interprétabilité & \cmark & Partiel & \cmark & \xmark \\
\hline
Risque de surapprentissage & \cmark & Partiel & \cmark & \xmark \\
\hline
...
\hline
Spécialisé RAG & Partiel & \cmark & Partiel & \xmark \\
\hline
Intégration Mistral & \cmark & \cmark & Partiel & \cmark \\
\hline
Stabilité de l’API & \xmark & \cmark & \cmark & \cmark \\
\hline
Adapté à un cas simple & Partiel & \cmark & Partiel & Partiel \\
\hline
Maintenance à long terme & Partiel & \cmark & \cmark & \xmark \\
\hline
\end{tabular}
...
\begin{longtable}{|p{3cm}|p{2.8cm}|p{10cm}|}
...
% Serveur ASGI & Gunicorn + UvicornWorker & Gunicorn orchestre les workers, UvicornWorker assure le support ASGI natif pour Django Ninja \\
% \hline
...
Monitoring d'erreurs & Sentry & Capture automatique des exceptions backend et frontend avec contexte de la requête \\
\hline

Métriques système & Prometheus + Grafana & Suivi des métriques de performance (CPU, mémoire, temps de réponse) sur l'infrastructure de production \\
\hline

Tests de charge & Locust & Simulation de trafic concurrent pour valider les cibles de performance (200~ms / 50 utilisateurs) \\
\hline

Audit sécurité & OWASP ZAP & Analyse automatisée des vulnérabilités web (XSS, injection, exposition de tokens) en fin de sprint \\
\hline
...
Les besoins du système sont désormais définis et les choix technologiques validés. Le chapitre suivant entre dans la phase de conception, en détaillant le product backlog Scrum, l’architecture du système, le modèle de données et les machines à états.
