...
La figure~\ref{fig:cas-utilisation} présente le diagramme de cas d'utilisation de la plateforme et permet de synthétiser les principaux besoins fonctionnels à travers les interactions avec le système. Deux utilisateurs principaux interviennent dans la plateforme : l'\textbf{étudiant}, dont l'accès est limité à ses propres données, et l'\textbf{administrateur RI}, chargé de la gestion et du suivi des différentes fonctionnalités. Deux utilisateurs secondaires interviennent également : le \textbf{CAS de Toulouse INP}, utilisé pour l'authentification, et les \textbf{systèmes sources} MoveON, Pégase et Eudonet, qui fournissent les données nécessaires à la plateforme. Les associations entre les utilisateurs et les cas d'utilisation représentent les principales interactions avec le système.

\begin{figure}[H]
  \centering
  \includegraphics[height=0.9\textheight]{figures/conception/Diag_cas_utilisation.png}
  \caption{Diagramme de cas d'utilisation de la plateforme}
  \label{fig:cas-utilisation}
\end{figure}

La lecture du diagramme fait ressortir la répartition des responsabilités. L'étudiant n'accède qu'à des cas de consultation et de déclaration relatifs à sa propre mobilité : catalogue de destinations, recommandations, interrogation de l'assistant virtuel, vœux, résultat d'affectation, mobilité complémentaire et suivi de sa durée de mobilité. L'administrateur RI concentre le pilotage : cycle de vie des années universitaires, référentiels, accords et quotas, synchronisation et réconciliation des imports, lancement et supervision de l'algorithme d'affectation, instruction des mobilités, finalisation CTI et consultation du journal d'audit. La synchronisation récupère les données de MoveON, Pégase et Eudonet via leurs API REST.

L'authentification, requise pour tous les cas d'utilisation, n'est pas répétée sur chacun sous forme de relation \og include\fg{} : elle est portée une seule fois par le cas \textit{S'authentifier} et rappelée par une note, pour ne pas surcharger le diagramme. Elle passe par le CAS de Toulouse INP, qui valide d'abord les identifiants institutionnels avant l'ouverture de la session. Enfin, le cas \textit{Interroger l'assistant virtuel} (BF10) relève des fonctionnalités secondaires (V2) et n'a pas été implémenté dans le cadre du PFE : il a été reporté.
